...
<div class="auth-page">
    <div class="auth-left">
        <div class="auth-left-content">
            ...
            <img src="<?= htmlspecialchars($basePath . '/public/assets/images/illustrations/recordando.png', ENT_QUOTES, 'UTF-8') ?>" class="hevy-mascot-reg" alt="Mascota Hevy">
    ...
    <div class="auth-right">
        ...
        <p class="auth-sub">
            ¿Ya tienes cuenta? <a href="<?= htmlspecialchars($basePath . '/login', ENT_QUOTES, 'UTF-8') ?>">Inicia sesión</a>
        </p>

        <form method="POST" id="form-registro" novalidate action="<?= htmlspecialchars($basePath . '/register', ENT_QUOTES, 'UTF-8') ?>">
            <div class="form-group">
                <label for="nombre_completo">Nombre completo</label>
                <input type="text" name="nombre_completo" id="nombre_completo" placeholder="Ej: Juan Pérez García" required autocomplete="name" value="<?= htmlspecialchars($_POST['nombre_completo'] ?? '') ?>">
            </div>

            <div class="form-group">
                <label for="email">Correo electrónico</label>
                <input type="email" name="email" id="email" placeholder="ejemplo@correo.com" required autocomplete="off" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>">
            </div>
    ...
    function openCaptcha() {
        ...
        if (!sc) {
            sc = new SliderCaptcha({
                ...
                onSuccess: () => { captchaVerified=true; overlay.classList.remove('visible'); window.hvShowSplash?.(); document.getElementById('form-registro').submit(); }
            });
        } else sc.reset();
    }
...
<script>
    ...
    window.FACE_API_UPDATE_STAGE = <?= json_encode($basePath . '/api/usuarios/actualizar_etapa') ?>;
</script>
...
<script>
    document.getElementById('face-register-toggle').addEventListener('click', () => {
        window.FaceAuth.openRegisterCapture((descriptor) => {
            ...
            setTimeout(() => { passWrap.style.display = 'none'; }, 300);
